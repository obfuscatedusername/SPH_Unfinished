...
///	Instanced Rendering Loader
model* modelLoader::loadModel(char* file, bool ins){
	theScene = aiImportFile(file, aiProcessPreset_TargetRealtime_Quality);
	if(!theScene){
		print->error("reading mesh ", file, 4);
		exit(1);
	...
	if(theModel->instanced)
		//printf("Model ready for Instanced Rendering!\n");
	//assign the number of materials and meshes from the scene to the model
	theModel->numMat = theScene->mNumMaterials;
	theModel->numMesh = theScene->mNumMeshes;
	//load the vertices, normals and textures for the model
...
void modelLoader::boneTransform(float secs, vector<Matrix_4f>& transforms, int anim, float& antime)
{
	...
	float animTime;
	if(anim==1)
		animTime = fmod(tit, 2.66666666667f); //duration of first anim	
	else if (anim==2) 
		animTime = fmod(tit, 7.03333333333f) + 3.2f; //duration of 2nd anim + start time of 2nd anim	
	else 
		animTime = fmod(tit, 6.0f) + 10.9333333333f; //duration of 3rd anim + start time of 3rd anim
...
GLuint modelLoader::loadTex(std::string fn){
	GLuint  imageID;
	ILboolean	success;
	//Initialise DevIL
	ilInit();
	...
	//load the image
	success = ilLoadImage((ILstring)fn.c_str());
	ilConvertImage(IL_RGBA, IL_UNSIGNED_BYTE);
	if(!success){
		printf("Could not properly load %s, Jabroni!\n", fn.c_str());
		ilDeleteImages(1, &imageID);
	}else{
		//printf("Successfully loaded %s, generating texture ting\n", fn.c_str());
